...
def list_to_colormap(x_list):
    ...
    for index, item in enumerate(x_list):
        if item == 1:
            y[index] = np.array([86, 255, 86]) / 255.
        if item == 2:
            y[index] = np.array([136, 156, 151]) / 255.
        if item == 3:
            y[index] = np.array([255, 250, 255]) / 255.
    return y
...
def get_cls_map(net, device, all_data_loader, y, name, epochs):
    ...
    classification_map(y_re, y, 300,
                       'classification_maps/' + name + '_predictions.eps')
    classification_map(y_re, y, 300,
                       'classification_maps/' + name + '_predictions' + '_' + str(epochs) + '.png')
    classification_map(gt_re, y, 300,
                       'classification_maps/' + name + '_gt.png')
